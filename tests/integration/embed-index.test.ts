...
import {
  ...
  getMetaPath,
  getNamespaceDir,
} from '../../src/embeddings/embedding-namespace.js'
import type { VectorEntry } from '../../src/embeddings/types.js'
import { createTestVectorStore } from '../../src/embeddings/vector-store-test-fixture.js'
...
const VECTOR_PROVIDER = 'openai'
const VECTOR_MODEL = 'text-embedding-3-small'
const VECTOR_DIMENSIONS = 512
const VECTOR_STORE_IDENTITY = {
  provider: VECTOR_PROVIDER,
  model: VECTOR_MODEL,
} as const

const vectorNamespace = (dimensions = VECTOR_DIMENSIONS): string =>
  generateNamespace(VECTOR_PROVIDER, VECTOR_MODEL, dimensions)

const vectorMetaPath = (indexRoot: string): string =>
  getMetaPath(indexRoot, vectorNamespace())

const vectorJsonPath = (indexRoot: string): string =>
  path.join(getNamespaceDir(indexRoot, vectorNamespace()), 'vectors.meta.json')
...
describe('Small Corpus Tests', () => {
  ...
  it('verifies binary format is used for vector metadata', async () => {
    ...
    const metaPath = vectorMetaPath(tempDir)
    const jsonPath = vectorJsonPath(tempDir)
...
describe('Large Corpus Tests', () => {
  ...
  it('large corpus can be loaded after indexing', async () => {
    ...
    expect(docIndex).not.toBeNull()
    expect(sectionIndex).not.toBeNull()
    expect(Object.keys(docIndex!.documents).length).toBeGreaterThanOrEqual(1000)
...
describe('Metadata and Binary Format Tests', () => {
  it('verifies vectors.meta.bin is created not vectors.meta.json', async () => {
    ...
    const binPath = vectorMetaPath(tempDir)
    const jsonPath = vectorJsonPath(tempDir)
  ...
  it('binary metadata can be loaded after saving', async () => {
    ...
    const vectorStore1 = createTestVectorStore(
      tempDir,
      512,
      VECTOR_STORE_IDENTITY,
    )
    ...
    // Load vector store
    const vectorStore2 = createTestVectorStore(
      tempDir,
      512,
      VECTOR_STORE_IDENTITY,
    )
...
describe('Vector Store Loading Tests', () => {
  ...
  it('detects dimension mismatch on load', async () => {
    ...
    const metaPath = vectorMetaPath(tempDir)
    const meta = decode(fs.readFileSync(metaPath)) as Record<string, unknown>
    meta.dimensions = 768
    fs.writeFileSync(metaPath, encode(meta))
    ...
    await expect(Effect.runPromise(vectorStore2.load())).rejects.toThrowError()
  })
  ...
  it('handles HNSW parameter mismatch detection', async () => {
    ...
    const vectorStore1 = createTestVectorStore(tempDir, 512, {
      ...VECTOR_STORE_IDENTITY,
    ...
    const vectorStore2 = createTestVectorStore(tempDir, 512, {
      ...VECTOR_STORE_IDENTITY,
