...
it('selects hybrid mode and projects overlapping channels', () => {
  ...
  const channels: SearchChannels = {
    semanticResults: [{ ...shared, similarity: 0.9 }],
    keywordResults: [{ ...shared, score: 4, rank: 1 }],
    hasEmbeddings: true,
    hasBM25: true,
  }
  const effective = selectEffectiveMode(undefined, channels)
  const projected = projectSearchResults(effective.mode, channels, {
    limit: 10,
    bm25Weight: 1,
    semanticWeight: 1,
    rrfK: 60,
  ...
  const result = {
    ...
    stats: { modeReason: effective.reason },
  }

  expect(result.results[0]?.sources).toEqual(['semantic', 'keyword'])
  expect(result.stats.modeReason).toBe('both indexes available')
})
...
describe('RRF Scoring', () => {
  ...
  it('should give higher scores to results found by both methods', async () => {
    ...
    if (bothSources.length > 0 && singleSource.length > 0) {
      ...
      const avgSingle =
        singleSource.reduce((sum, r) => sum + r.score, 0) / singleSource.length
